...
for N in "$@"; do
    ...
    find . -name '*.xhtml' -exec sed -i "s/′/´/g;s/ʹ/´/g;s/‑/-/g;s,<div class=\"[^\"]*w_navigation[^\"]*\"> *\\(<a href[^>]*>[^<]*</a>\\)</div>, \\1,g;s,<div class=\"[^\"]*w_navigation[^\"]*\"> *\\(\\( *<a href[^>]*>[^<]*</a>\\)*\\)</div>, \\1,g" '{}' + # assumes GNU sed for -i behaviour
